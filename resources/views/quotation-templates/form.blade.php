...
<div class="page-body"><div class="container-xl"><form class="card" method="POST" action="{{ $isEdit ? route('quotation-templates.update', $template) : route('quotation-templates.store') }}">
    ...
    <div class="card-body"><div class="row g-3">
        ...
        <div class="col-md-6"><label class="form-label" for="document_type_id">Document type untuk numbering</label><select class="form-select" id="document_type_id" name="document_type_id" required><option value="">Pilih document type</option>@foreach($documentTypes as $documentType)<option value="{{ $documentType->getKey() }}" @selected(old('document_type_id', $template->document_type_id) === $documentType->getKey())>{{ $documentType->name }} ({{ $documentType->code }}) — {{ $documentType->number_pattern }}</option>@endforeach</select><div class="form-hint">Nomor quotation yang memakai template ini akan mengikuti sequence document type terpilih.</div></div>
