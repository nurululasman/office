...
<div class="card-body"><div class="row g-3">
    ...
    <div class="col-12"><label class="form-label" for="bank_information">Bank information</label><textarea class="form-control" id="bank_information" name="bank_information" rows="5" maxlength="10000">{{ old('bank_information', $profile->bank_information) }}</textarea><div class="form-hint">Opsional. Baris baru dipertahankan melalui placeholder <code>company_bank_information</code>.</div></div>
    <div class="col-md-6"><label class="form-label" for="logo">Logo PNG/JPEG</label><input class="form-control" type="file" id="logo" name="logo" accept="image/png,image/jpeg"><div class="form-hint">Maksimum 2 MB. Upload baru tidak menghapus file lama agar snapshot tetap dapat direproduksi.@if($profile->logo_path) File saat ini: <a href="{{ $profile->logo_path }}" target="_blank" rel="noopener">Lihat logo</a>@endif</div></div>
    <div class="col-md-6"><label class="form-label" for="stamp">Stamp / Cap Perusahaan PNG/JPEG</label><input class="form-control" type="file" id="stamp" name="stamp" accept="image/png,image/jpeg"><div class="form-hint">Maksimum 2 MB. Disarankan format PNG dengan latar transparan.@if($profile->stamp_path) File saat ini: <a href="{{ $profile->stamp_path }}" target="_blank" rel="noopener">Lihat stamp</a>@endif</div></div>
