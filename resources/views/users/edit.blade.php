...
@section('title', auth()->user()->is($managedUser) ? 'Profil & Tanda Tangan Saya' : 'Edit User & Atur Akses')
@section('content')
<div class="page-header d-print-none">
    <div class="container-xl">
        <div class="page-pretitle">{{ auth()->user()->is($managedUser) ? 'Pengaturan Akun' : 'Administrasi user' }}</div>
        <h1 class="page-title">{{ auth()->user()->is($managedUser) ? 'Profil & Tanda Tangan Saya' : 'Edit User: ' . $managedUser->name }}</h1>
    </div>
</div>
<div class="page-body">
    <div class="container-xl">
            ...
            <div class="col-lg-8">
                <form class="card" method="POST" action="{{ route('users.update', $managedUser) }}" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        @if(auth()->user()->is($managedUser) && !$managedUser->signature_path)
                            <div class="alert alert-info mb-3">
                                <strong>Lengkapi Profil:</strong> Silakan periksa nama Anda dan unggah foto tanda tangan untuk keperluan pembuatan dan persetujuan quotation.
                            </div>
                        @endif
                        <div class="row g-3">
                            <div class="col-md-4">
                    ...
                    <div class="card-footer text-end">
                        <a href="{{ auth()->user()->can('viewAny', App\Models\User::class) ? route('users.index') : route('office.home') }}" class="btn me-2">Batal</a>
